...
router.get('/current/company', auth, async(req, res)=>{
     ...
     try {
         if(!profile){
             return res.json({msg: 'You must Build A Company To Get Profile'});
         }
...
router.get('/all',  async(req, res)=>{
    ...
    try {
        ...
        res.status(200).json({data: { profiles, products }})
    } catch (err) {
        console.log(err.message);
        res.status(500).send('Internal Server Error')
...
try {
    ...
    new Profile(profileFields).save().then(p => res.json({ profile: p }))      
} catch (err) {
    console.log(err.msg);
    res.status(500).send('Internal Server Error ')
}
...
router.post('/like/:id',auth, async(req, res)=>{
    ...
    try {
        if(profile.likes.filter(like=>like.user.toString()===req.user.id).length>0){
          ...
          .save()
          .then(profile =>{ 
            res.status(200).json(profile.likes)
            io.getIO().emit('aime', { action:'add-likes', likes: profile.likes })
        }) 
